...
test('MEXC-only MX pair recovers from an outage without interrupting Binance', async ({ page }) => {
  ...
  await expect(page.locator('.grid-caption .live-dot')).not.toHaveClass(/muted-dot/);
  await expect(page.locator('.grid-caption')).toContainText('CHART & CHANGE · 24H');
  ...
  await expect(page.locator('.grid-caption')).toContainText('LAST KNOWN PRICES');
  await expect(page.locator('.grid-caption .live-dot')).toHaveClass(/muted-dot/);
