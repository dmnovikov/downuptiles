...
interface Props { id: string; editing: boolean; readOnly?: boolean; snapshot?: Quote; snapshotStale?: boolean; onOpen: (id: string) => void; onEdit: () => void; onRemove: (id: string) => void; onMove: (id: string) => void }
export const QuoteTile = memo(function QuoteTile({ id, editing, readOnly = false, snapshot, snapshotStale = false, onOpen, onEdit, onRemove, onMove }: Props) {
  const provider = providerFor(id);
  const asset = provider.getAsset(id), storedQuote = useQuote(id), error = useQuoteError(id);
  const connection = useConnection(id);
  const quote = snapshot ?? storedQuote;
  const unavailable = snapshot ? snapshotStale : Boolean(error) || connection === 'offline' || connection === 'error';
  const stale = snapshot ? snapshotStale : Boolean(quote) && (unavailable || connection === 'connecting');
  const refreshing = !snapshot && stale && connection === 'connecting';
