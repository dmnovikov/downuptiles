...
import { useQuote, useQuoteError } from '../hooks/useMarket';
...
export const QuoteTile = memo(function QuoteTile({ id, editing, onOpen, onEdit, onRemove, onMove }: Props) {
  ...
  const color = quote ? direction(quote.change24h) : 'flat', dots = quote ? changeDots(quote.change24h) : 0;
  const cancel = () => clearTimeout(press.current?.timer);
  return <article ref={setNodeRef} className={`quote-tile ${color} ${editing ? 'editing' : ''} ${isDragging ? 'dragging' : ''}`} style={{ transform: CSS.Transform.toString(transform), transition }} data-testid={`tile-${id}`}>
    <button className="tile-content" disabled={editing} aria-label={`Open chart for ${asset?.symbol ?? id}`} onContextMenu={event => event.preventDefault()}
      ...
      {quote ? <Sparkline points={quote.sparkline}/> : error ? <div className="tile-unavailable" title={error}>{error}</div> : <div className="sparkline loading-line"/>}
      <div className="tile-bottom"><span className="tile-change">{quote ? formatChange(quote.change24h) : '—'}</span><span className="period-label">24h</span></div>
    </button>
