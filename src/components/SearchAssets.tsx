...
export function SearchAssets({ selected, excluded, screenName, onAdd, onClose }: { selected: string[]; excluded: string[]; screenName: string; onAdd: (id: string) => void; onClose: () => void }) {
  ...
  const available = assets.filter(asset => !hidden.has(asset.id) || selected.includes(asset.id));
  return <Modal title="Add asset" onClose={onClose}>
    ...
    {excluded.length > 0 && <p className="muted modal-description">Pairs in other tabs are hidden.</p>}
    ...
    <div className="asset-results" aria-busy={loading}>
      ...
      {!loading && !error && !available.length && <p className="empty-small">No available assets found</p>}
      {available.slice(0, limit).map(asset => {
      ...
      {!loading && available.length > limit && <button className="secondary-button full-width" onClick={() => setLimit(n => n + 20)}>Show 20 more</button>}
    </div>
    {!loading && available.length > 0 && <p className="search-count">{Math.min(limit, available.length)} of {available.length} pairs · Type to narrow results</p>}
