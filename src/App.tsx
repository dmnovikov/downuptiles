...
export default function App() {
  ...
  const openMovers = () => { history.pushState({ movers: true }, '', '#/movers'); setWorld({ active: false, id: null }); setMoversTab('my'); setMovers(true); };
  ...
  return <div className="app-shell">
    ...
    {!world.active && (connection === 'offline' || connection === 'error' || connection === 'polling') && <div className="connection-banner" role="status"><WifiOff size={14}/>{connection === 'offline' ? 'Offline · Showing last known prices' : connection === 'polling' ? 'Updates every 5 seconds' : 'Prices may be outdated · Reconnecting'}</div>}
    {world.id ? <Suspense fallback={<div className="page-loading">Loading chart…</div>}><WorldDetail key={world.id} id={world.id} onBack={backWorld}/></Suspense> : assetId ? connection === 'offline' && !chartLoaded ? <div className="empty-state"><WifiOff size={30}/><h2>Chart not downloaded yet</h2><p>It will load when you are back online.<br/>Your watchlists are still available.</p><button className="primary-button" onClick={back}>Back to watchlist</button></div> : <Suspense fallback={<div className="page-loading">Loading chart…</div>}><ChartPage assetId={assetId} onBack={back} backLabel={history.state?.chartFrom === 'movers' ? 'Back to Top movers' : 'Back to watchlist'}/></Suspense> : movers ? <TopMovers tab={moversTab} onTab={changeMoversTab} ids={allIds} onBack={backFromMovers} onOpen={openAsset}/> : <>
      <header className="app-header"><a className="brand" href="#/movers" onClick={event => { event.preventDefault(); openMovers(); }} aria-label="downuptiles · Top movers" title="Top movers"><img className="brand-logo" src="/logo.png" alt="" width="30" height="30"/><span>downup<span>tiles</span></span></a><div className="header-actions"><div className="market-switch" role="group" aria-label="Market type"><button aria-pressed={!world.active} onClick={() => { if (world.active) setMarketMode(false); }}>Crypto</button><button aria-pressed={world.active} onClick={() => { if (!world.active) setMarketMode(true); }}>Market</button></div><button className="icon-button" aria-label="Settings" title="Settings" onClick={() => setModal('settings')}><SettingsIcon size={19}/></button></div></header>
      {world.active ? <WorldMarket onOpen={openWorldAsset}/> : <main aria-label="Quotes">
